<template>
    <Modal v-show="this.$store.state.isModal">
        <div class="delete-modal" v-show="typeOfModal===1">
            <h2 >Удалить запись?</h2>
            <button @click="okDeleteItem"> Удалить</button>
            <button @click="this.$store.state.isModal=false">
                Отмена
            </button>
        </div>
        <h2 v-show="this.$store.state.typeOfModal===2">Введите непустое значение</h2>
...
<script>
    ...
    export default {
        ...
        methods:{
            editMode(itemId){
                let value={
                    idOfItem:this.idOfItemInArray,
                    taskId:itemId
                }
                this.$store.commit('setEditModePage',value);
            ...
            editDone(itemId){
                if(this.$refs[`item_${itemId}`][0].value.trim()!==''){
                    ...
                    this.$store.commit('setEditModeDonePage',valId);
                    this.todoArray=this.$store.state.taskArray[this.idOfItemInArray].taskList;
                    this.editLocalArray();
                }
                else{
            ...
            makeComplete(itemId){
                ...
                this.$store.commit('setCompletePage',value);
                this.editLocalArray();
            },
            deleteItem(itemId){
               this.$store.state.isModal=true;
            ...
            okDeleteItem(){
                let value={
                    idOfItem:this.idOfItemInArray,
                    taskId:this.deleteItemId
                }
                this.$store.commit('deleteItemPage',value);
                this.editLocalArray();
                this.$store.state.isModal=false
        ...
        mounted() {
            this.$store.commit('setLocalArray',JSON.parse(localStorage.getItem('tasksList')));
            this.taskTodoArray=this.$store.state.taskArray;
            for(let i=0;i<this.taskTodoArray.length;i++){
               if(this.taskTodoArray[i].id==parseInt(this.$route.params.id)) {
                   this.idOfItemInArray=i;
                   this.todoArray=this.taskTodoArray[i].taskList;
               }
            }
